...
## Process Overview
The merge process uses a **chunked approach** to reduce complexity and improve success rates. Instead of merging entire upstream versions at once, commits are processed in small batches with CI validation.
...
4. **Identify merge range and group commits:**
    Use .\tools\Get-CommitGroups.ps1
   ```pwsh
   # Find the last upstream tag in the fork (this is the starting point for the next merge)
   # Call .\tools\Get-CommitGroups.ps1 -GitHubTag <last-upstream-tag> -FirstChunkOnly
   # Example output:
   # {
   #   "ChunkNumber": 1,
   #   "StartCommit": "609fe2c",
   ...
   # Follow the below steps to completion
   # Call .\tools\Get-CommitGroups.ps1 -StartCommit <end-commit-sha> -FirstChunkOnly
   # Follow the below steps to completion
   # Continue to call .\tools\Get-CommitGroups.ps1 -StartCommit <end-commit-sha> -FirstChunkOnly
   # with the end-commit-sha of the previous chunk and
...
9. **Commit batch completion:**
   ```pwsh
   git commit -m "Merge upstream commits batch 1: $($commitBatches[0].Split(' ')[0])..$($commitBatches[-1].Split(' ')[0])

   Commits included:
   $(foreach ($commit in $commitBatches) { "- $commit" })

   Major conflict resolutions:
   - <file1>: <resolution strategy and reasoning>
   - <file2>: <resolution strategy and reasoning>
   - <file3>: <resolution strategy and reasoning>"
   ```
