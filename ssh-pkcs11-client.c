...
#ifdef WINDOWS
/*
 * On Windows the helper is spawned as the connecting user (via the ssh-agent
 * client token) rather than fork()/execlp(), and is located relative to the
 * running module instead of $PATH. These globals are provided by the Windows
 * ssh-agent (contrib/win32/win32compat/ssh-agent/agent.c).
 */
...
#ifdef WINDOWS
/*
 * Windows spawns the helper as the connecting user using the ssh-agent
 * client's primary token (posix_spawnp_as_user) instead of fork()/execlp(),
 * and locates the helper relative to the running module.
 */
static struct helper *
pkcs11_start_helper(const char *path)
{
	...
	av[1] = verbosity;
	av[2] = NULL;
	debug_f("starting %s %s", prog, verbosity == NULL ? "" : verbosity);

	if (sshagent_con_username != NULL) {
		debug_f("spawning helper as user %s", sshagent_con_username);
		if (sshagent_client_primary_token == NULL) {
			error_f("sshagent_client_primary_token is NULL for "
			    "user %s", sshagent_con_username);
			goto fail;
		}
		if (posix_spawnp_as_user(&pid, av[0], &actions, NULL, av, NULL,
		    sshagent_client_primary_token) != 0) {
			error_f("failed to spawn process %s", av[0]);
			goto fail;
		}
	} else {
		if (posix_spawnp(&pid, av[0], &actions, NULL, av, NULL) != 0) {
			error_f("posix_spawnp %s failed", av[0]);
			goto fail;
		}
	}
